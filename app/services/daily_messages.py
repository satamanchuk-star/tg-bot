...
# ---------------------------------------------------------------------------
# Напоминания о присутствии: подчёркивают, что бот отвечает ТОЛЬКО по запросу.
# Утром (10:00) и вечером (21:00) в главном чате (General), варьируются.
# ---------------------------------------------------------------------------

_PRESENCE_MORNING = (
    "Доброе утро, соседи! ☕ Я на месте — по чату молча дежурю. Понадоблюсь — "
    "просто позовите «Жабот» или ответьте на моё сообщение, и я тут же помогу.",
    "Утро доброе! Напоминаю: сам в разговоры не лезу, но всегда на связи. "
    "Нужен совет или справка — окликните «Жабот», разберёмся.",
    "С добрым утром! 🌅 Живу тихо, встреваю только по приглашению. Есть вопрос "
    "по дому, УК или шлагбауму — позовите меня по имени, отвечу.",
    "Доброе утро! Я весь день на посту, но без лишнего шума. Что-то узнать — "
    "напишите «Жабот, …» или ответом на моё сообщение.",
    "Утречко, соседи! ☀️ Я рядом и готов помочь — но только по запросу. Зовите "
    "«Жабот», когда понадоблюсь, а до тех пор не мешаю.",
    "Доброе утро! Молчаливый сосед снова на дежурстве. Нужна подсказка — "
    "кликните «Жабот» или ответьте реплаем на это сообщение.",
)

_PRESENCE_EVENING = (
    "Всем спокойной ночи, соседи! 🌙 Я на дежурстве и ночью — если что-то "
    "понадобится, позовите «Жабот», отвечу даже в темноте.",
    "Доброй ночи! 😴 Двор под присмотром, я на связи круглосуточно. Нужна помощь "
    "среди ночи — просто окликните меня по имени.",
    "Спокойной ночи всем! Я никуда не ухожу — дежурю с ответами и ночью. "
    "Понадоблюсь — «Жабот» или реплай на моё сообщение.",
    "Ночь добрая, соседи! 🌚 Отдыхайте, а я послежу. Если что-то нужно — зовите "
    "«Жабот» в любой час, я не сплю.",
    "Спокойной ночи! Лифт, шлагбаум и я работаем без выходных. Спросить что-то "
    "ночью? Позовите «Жабот», отвечу.",
    "Доброй ночи, дорогие соседи! 🌙 Дежурю до утра. Нужен — окликните по имени, "
    "помогу в любое время.",
)

_LAST_PRESENCE_IDX: dict[str, int] = {}


def _pick_varied(pool: tuple[str, ...], key: str) -> str:
    """Выбирает фразу из пула, не повторяя предыдущую подряд."""
    idx = random.randrange(len(pool))
    if len(pool) > 1 and _LAST_PRESENCE_IDX.get(key) == idx:
        idx = (idx + 1) % len(pool)
    _LAST_PRESENCE_IDX[key] = idx
    return pool[idx]


async def send_presence_morning(bot: Bot) -> None:
    """10:00 — напоминание в главном чате, что бот отвечает только по запросу."""
    try:
        await bot.send_message(settings.forum_chat_id, _pick_varied(_PRESENCE_MORNING, "morning"))
        logger.info("PRESENCE: утреннее напоминание отправлено")
    except Exception:
        logger.warning("Не удалось отправить утреннее напоминание.", exc_info=True)


async def send_presence_evening(bot: Bot) -> None:
    """21:00 — вечернее «спокойной ночи, дежурю и ночью» в главном чате."""
    try:
        await bot.send_message(settings.forum_chat_id, _pick_varied(_PRESENCE_EVENING, "evening"))
        logger.info("PRESENCE: вечернее напоминание отправлено")
    except Exception:
        logger.warning("Не удалось отправить вечернее напоминание.", exc_info=True)
