...
HELP_MENU_TEXT = (
    ...
    "• /meme /poster /rules_image — генерация картинок через AI\n"
    "• /admin_reply_ai — вежливый ответ на обращение\n"
    "• /explain_error — объяснить техническую ошибку\n\n"
...
- Мягкие подсказки куда лучше написать, если сообщение попало не в ту тему
...
- Умное определение проблем со шлагбаумом и сбор данных для заявки

ГЕНЕРАЦИЯ КАРТИНОК (только для администраторов):
- /meme — мем про жизнь ЖК
- /poster — афиша мероприятия
- /digest_image — иллюстрация к сводке новостей
- /rules_image — иллюстрация правила чата
- /welcome_card — открытка для новых соседей
- /warning_image — заметное предупреждение

ИНСТРУМЕНТЫ ДЛЯ АДМИНИСТРАТОРОВ:
- /admin_reply_ai — сгенерировать вежливый ответ на жалобу или обращение
- /explain_error — объяснить техническую ошибку
- /mute, /ban, /strike — модерация пользователей
"""
