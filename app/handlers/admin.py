...
async def _ensure_admin(message: Message, bot: Bot) -> bool:
    if message.from_user is None:
        # Анонимные посты от группы (sender_chat == сам форум) в Telegram могут
        # делать только админы группы, поэтому мы им доверяем. Но для анонимных
        # постов из других чатов (например, чужой канал, переадресация) доступ не
        # выдаём — не полагаемся на то, что Telegram строго фильтрует такие случаи.
        if message.sender_chat and message.sender_chat.id == settings.forum_chat_id:
            return True
        if message.sender_chat:
            logger.info(
                "Отклонён админский запрос от sender_chat=%s (не совпадает с форумом).",
...
async def reload_profanity(message: Message, bot: Bot) -> None:
    ...
    await message.reply(
        "Словари перечитаны с диска и применены. "
        f"Мат-словарь: {words_count}, исключения: {exceptions_count}."
    )
