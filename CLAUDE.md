...
- Вопросы — `data/quiz_questions.json` → `scripts/seed_quiz.py`; пул не истощается (recycle).
- Пополнение пула из веба: админ `/quiz_import <url>` — качает страницу и извлекает
  пары Q/A силами ИИ бота (`app/services/quiz_import.py`). Работает в рантайме
  сервера (там есть интернет), а не в окружении сборки.
