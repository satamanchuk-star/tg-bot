...
- Конвейер вопросов: `data/quiz_questions.json` (286 шт.) → **валидация** (`scripts/validate_quiz.py`:
  форма вопроса + ответ обязан матчиться сам себе) → `scripts/seed_quiz.py` → игра.
  CI-гейт: `tests/test_quiz_questions_valid.py`. Пул не истощается (recycle).
- Пополнение с квиз-сайтов: `scripts/parse_quiz_page.py` — детерминированный парсер
  формата «нумерованные вопросы → „Ответы“ → нумерованные ответы» (пары по номерам,
  кривые ответы отсеивает валидатор). Запускать там, где сайты доступны (сервер/локально);
  из окружения сборки эти домены закрыты egress-политикой.
...
- Достоверность: кнопка «⚠️ Устарело» под ответами → лог-чат; вторничная
  авто-сверка `place_verify`; `/kb_stale` — отчёт о несвежих данных.
