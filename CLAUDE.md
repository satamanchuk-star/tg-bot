...
- Петля роста персистентна: привязка «сообщение дайджеста → вопрос» хранится
  в `unanswered_questions.digest_message_ids` (переживает рестарт); ответ
  реплаем работает прямо на вопрос дайджеста. У всех записей KB есть
  `verified_at`; `/kb_stale` перечисляет несвежие записи KB, а вторничная
  сверка `place_verify` проверяет и URL из ответов KB.
